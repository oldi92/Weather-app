...
export const currentWeather = (longitude, latitude) => {
  return (dispatch) => {
    ...
    axios
      .get(
        `https://api.weatherbit.io/v2.0/current?lat=${lat}&lon=${lon}&key=3fab93ee25494833b253c5e6e3eff0a3`
      )
      .then((response) => {
...
export const forecastFetch = (longitude, latitude) => {
  return (dispatch) => {
    ...
    axios
      .get(
        `https://api.weatherbit.io/v2.0/forecast/hourly?&lat=${lat}&lon=${lon}&key=3fab93ee25494833b253c5e6e3eff0a3&hours=12`
      )
      .then((response) => {
...
export const forecast5Fetch = (longitude, latitude) => {
  return (dispatch) => {
    ...
    axios
      .get(
        `https://api.weatherbit.io/v2.0/forecast/daily?lat=${lat}&lon=${lon}&key=3fab93ee25494833b253c5e6e3eff0a3&days=5`
      )
      .then((response) => {
        console.log(response.data);
...
export const signInAuth = (email, password) => {
  return (dispatch) => {
    ...
    axios
      .post(
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDCA2dGQrKkk2VExEMTtatIKCVEw1M1FfI",
        data
      )
      .then((response) => {
...
export const signUpAuth = (email, password) => {
  return (dispatch) => {
    ...
    axios
      .post(
        "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDCA2dGQrKkk2VExEMTtatIKCVEw1M1FfI",
        data
      )
      .then((response) => {
        const personalData = {
        ...
        axios
          .put(
            `https://weather-app1-1bb76.firebaseio.com/${response.data.localId}.json`,
            {
...
export const personalDataEditFetch = (personalData, localId) => {
  return (dispatch) => {
    axios
      .put(`https://weather-app1-1bb76.firebaseio.com/${localId}.json`, {
        id: localId,
        personalData,
      })
      .then((response) => {
...
export const personalInfoFetch = (localId) => {
  return (dispatch) => {
    axios
      .get(
        `https://weather-app1-1bb76.firebaseio.com/${localId}/personalData.json`
      )
      .then((response) => {
