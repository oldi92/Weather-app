    ...
    render(){ 
        ...
        if(this.props.personalDataLoaded){
            if(this.props.personalData){
                personalInfoDiv = (
                    <ul>
                    <li><h3>First Name</h3><h2>{this.props.personalData.name 
                                            ? this.props.personalData.name 
                                            : 'Please edit name' }</h2></li>
                    <li><h3>Last Name</h3><h2>{this.props.personalData.lastName 
                                            ? this.props.personalData.lastName
                                            : 'Please edit name'}</h2></li>
                    <li><h3>Country</h3><h2>{this.props.personalData.country
                                            ? this.props.personalData.country
                                            : 'Please edit country'}</h2></li>
                    </ul>
                ) 
            }else {
                this.props.onPersonalInfoFetch(this.props.localId)
                personalInfoDiv = (
                    <ul>
                    <li><h3>First Name</h3><h2>No Name</h2></li>
                    <li><h3>Last Name</h3><h2>No Last Name</h2></li>
                    <li><h3>Country</h3><h2>No Country</h2></li>
                    </ul>
                )
            }
        ...
        return(
            ...
            <Transition in={this.props.personalDataToggle} timeout={200}>
                {state => (
                <div>
                <div className={classes.PersonalInfo} style={{
                    position: 'absolute',
                ...
                <div className={classes.PersonalData} style={{
                    position: 'absolute',
                    ...
                    <FontAwesomeIcon icon={faCheck} /> Save Changes
                        ...
                        <label>
                            Country <br/>
                        <input type="text" name="country" onChange={this.inputHandler}/>
                        </label>
                    </form>
                </div>
                </div>
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        onPersonalDataEditFetch : (personalData, localId) => dispatch(actions.personalDataEditFetch(personalData, localId)),
        onPersonalInfoFetch : (localId) => dispatch(actions.personalInfoFetch(localId)),
        onPersonalDataLoaded : () => dispatch(actions.personalDataLoaded()),
        onSetLocalIdIdtoken: (idToken , localId) => dispatch(actions.setLocalIdIdtoken(idToken , localId))
    }
}
